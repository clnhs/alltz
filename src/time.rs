...
impl TimeZone {
    ...
    pub fn from_tz(tz: Tz) -> Self {
        ...
        let display_name = cities_data.cities.iter()
            .find(|city| {
...
impl TimeZoneManager {
    ...
    pub fn get_all_available_timezones() -> Vec<(Tz, String, String, f64, f64)> {
        let cities_data = Self::load_cities_data();
        cities_data.cities.iter().map(|city| {
            let tz = Tz::from_str(&city.timezone).expect("Invalid timezone in cities.json");
            (tz, city.name.clone(), city.code.clone(), city.coordinates[0], city.coordinates[1])
        }).collect()
    }
    ...
    pub fn add_timezone_by_name(&mut self, name: &str) -> bool {
        if let Some(city) = self.cities_data.cities.iter().find(|c| c.name.eq_ignore_ascii_case(name)) {
    ...
    pub fn with_default_zones() -> Self {
        ...
        let default_city_names = vec![
            "Los Angeles", "New York", "UTC", "London", "Berlin", "Tokyo", "Sydney"
        ];
        
        let mut zones: Vec<TimeZone> = default_city_names
            .into_iter()
            .filter_map(|name| {
                cities_data.cities.iter().find(|c| c.name == name).and_then(|city| {
                    Tz::from_str(&city.timezone).ok().map(|tz| {
                        TimeZone::new(tz, tz.to_string(), city.code.clone())
                    })
                })
            })
            .collect();
